  ...
  if (
    ...
    getOpenTabsCount(function (count) {
      ...
      if (newUniqueCount > uniqueCount) {
        ...
        const getExperienceCountAPI = async (userToken, experienceID) => {
          try {
            const res = await fetch(
              // fetch (url,{})
              "http://localhost:3005/experienceCounter/opentabsandthiercounts", //TODO: can usr names be upperCase
              {
                method: "POST",
                body: JSON.stringify({
        ...
        const sosis2 = async () => {
          let sososi = await opentabsandthiercounts(
            "usrT5SD5FG33@5OCT2021HRW.eu",
            "5OCT2021HRW"
          );
          console.log(sososi.count);
        };
        sosis2();
      }
    });
